...
- [Firefox サイドバーの設定](#️-firefox-サイドバーの設定)
- [v6 より前のバージョンからアップデートする](#-v6-より前のバージョンからアップデートする)
...
### 「カーソルを合わせた時にサイドバーを展開する」を有効にすると、FlexFox が正しく動作しないのはなぜですか？

FlexFox は独自のサイドバー自動非表示機能とレイアウト切り替え機能を使用しています。Firefox の「**カーソルを合わせた時にサイドバーを展開する**」オプションはサイドバーの動作を変更するため、FlexFox の一部機能と競合する場合があります。

このオプションはデフォルトで無効になっており、最適な動作のため、オフのままにしておくことを推奨します。

設定を確認するには、ナビゲーションバーの **サイドバーボタン**（Firefox ロゴ）を右クリックし、「**サイドバーのカスタマイズ**」を選択して、「**カーソルを合わせた時にサイドバーを展開する**」がオフになっていることを確認してください。

---

## 🧹 v6 より前のバージョンからアップデートする

### Sidebery のスタイルエディターから古い FlexFox スタイルを削除する必要があるのはなぜですか？

v6 より前の FlexFox では、`sidebery-styles.json` を使用して、Sidebery のスタイルエディター経由でカスタム CSS を適用していました。

v6 以降、FlexFox に含まれる Sidebery 用スタイルはすべて `uc-sidebery.css` に移行されました。これまでスタイルエディターに追加していた旧スタイルは、現在は使用されていません。

現時点では旧スタイルとの互換性は維持されていますが、重複したスタイル処理や将来のアップデートとの競合を避けるため、削除することを強く推奨します。
