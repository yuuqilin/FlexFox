...
| Preference | Value | Description |
|-----------|:-------------:|-------------|
| `uc.flex.show-flexfox-version-info-in-about-config` | `true` | Displays the installed FlexFox version in `about:config`, making it easy to compare with the latest release and determine whether an update is available. |
