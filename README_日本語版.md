...
* ネイティブ垂直タブの折りたたみモード時のレイアウトに対応しました。折りたたみモードはサイドバーボタンから切り替えられます（アイコンがグレースケール表示になり、サイドバーは展開されなくなります）。
* 新しい設定項目 `uc.flex.sidebery-disable-icon-scaling` を追加しました。`true` に設定すると、Sidebery のピン留めタブおよびタブパネルのアイコン拡大表示を無効化できます。
* Sidebery の最上位タブが展開時に追加されていた余分なインデントを削除しました。 (#35)
* Sidebery の最上位タブにツリーガイドラインが二重表示される問題を修正しました。 (#35)
* macOS における Sidebar Button アイコンの配置および間隔の問題を修正しました。 (#35)
